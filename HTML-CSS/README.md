# YouTube UI Clone 🎥

A simple **YouTube homepage UI clone** built using only **HTML and CSS**.  
This project was created to practice frontend development fundamentals like layout design, responsive styling, and UI recreation without any JavaScript.

---

## 🚀 Features
- Clean and structured layout
- YouTube‑style navigation bar
- Video thumbnails grid
- Sidebar with categories
- Pure HTML & CSS (no frameworks)

---

## 📂 Project Structure

---

## 🛠️ Technologies Used
- **HTML5** for structure
- **CSS3** for styling (flexbox, grid, shadows, hover effects)

---

## 📸 Preview
![Screenshot of YouTube Clone](https://github.com/khushimishra0306/Web-Development/blob/main/HTML-CSS/Screenshot.png?raw=true)


---

## 📌 How to Run
1. Clone the repository:
   ```bash
   git clone https://github.com/khushimishra0306/Web-Development.git
